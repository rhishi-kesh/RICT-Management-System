    ...
    <div class="px-2 sm:px-16 w-full flex items-center justify-center">
        <div class="relative w-full md:w-2/4 rounded-md p-2 bg-gray-200 px-2 md:px-6 py-10">
            <div class="px-4 md:px-10">
                <div class="mb-10">
                    <h1 class="text-3xl font-extrabold uppercase !leading-snug text-primary md:text-4xl text-blue-500">Find Your Account</h1>
                    <p class="text-base">Please enter your email address to search for your account.</p>
                </div>

                @if (Session::has('error'))
                    <p class="p-2 rounded bg-red-500 text-white"> {{ Session::get('error') }} </p>
                @endif

                @if (Session::has('success'))
                    <p class="p-2 rounded bg-green-500 text-white"> {{ Session::get('success') }} </p>
                @endif

                <form action="{{ route('student.forgotPasswordPost') }}" class="space-y-5" method="POST">
                    @csrf
                    <div>
                        <div class="relative">
                            <input id="email" type="email" placeholder="Enter your email" class="ps-10 login-form-input bg-white focus:border-orange-500" name="email" value="{{ old('email') }}" />
                    ...
                    <button type="submit" name="submit" class="btn btn-gradient mt-2 w-full border-0 uppercase shadow hover:submit-btn focus:submit-btn active:submit-btn">
                        Varify Email
                    </button>
                </form>
                <a href="{{ route('studentLogin') }}" class="mt-2 inline-block hover:underline text-black text-xl">Sign in</a>
            </div>
        </div>
    </div>
</body>
