        ...
        <div class="relative w-full md:w-2/4 rounded-md p-2 bg-gray-200 px-2 md:px-6 py-10">
            <div class="px-4 md:px-10">
                <div class="mb-10">
                    ...
                    <p class="text-base">
                        We emailed you a six-digit code to <b>{{ $user->email }}</b>.
                        Enter the code below to confirm your email address.
                    </p>
                </div>
                <form action="{{ route('mentor.verifyPost') }}" method="post" id="verificationForm">
                    @csrf
                    <div>
                        <input type="hidden" name="email" value="{{ $user->email }}">
                        <div class="relative">
                            <input id="otp" type="text" placeholder="Enter OTP" class="ps-10 login-form-input bg-white focus:border-blue-700" name="otp" value="{{ old('otp') }}" />
            ...
            $.ajax({
                ...
                success:function(data){
                    if (data.status == 0) {
                        ...
                        window.location.href = "{{ route('mentor.changePassword', $user->id) }}";
                    }
                }
            });
