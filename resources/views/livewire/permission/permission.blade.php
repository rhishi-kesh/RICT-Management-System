<div class="pt-5" x-data="modal">
    ...
    <div class="bg-white dark:bg-slate-900 shadow-md rounded px-4 md:px-8 pt-6 pb-8 mb-4 w-full">
        <h2 class="mb-2 font-bold text-3xl dark:text-white text-blue-500">Permission</h2>
        <hr>
        <div>
            {{-- Show Data --}}
                ...
                <table class="w-full">
                    <thead>
                        <tr>
                            ...
                            <th class="p-3 bg-gray-100 dark:bg-gray-800 text-center">Group</th>
                    ...
                    <tbody>
                        @forelse ($permissions as $key => $data)
                            <tr>
                                <td class="p-3 border-b border-[#ebedf2] dark:border-[#191e3a] text-center">{{ $permissions->firstItem() + $key }}</td>
                                <td class="p-3 border-b border-[#ebedf2] dark:border-[#191e3a] text-center">{{ $data->group }}</td>
            ...
            <div class="livewire-pagination mt-5">{{ $permissions->links() }}</div>
        </div>
    </div>
        ...
        <div class="flex items-center justify-center min-h-screen px-4" @click.self="open = false">
            <div x-show="open" x-transition x-transition.duration.400 class="panel border-0 p-0 rounded-lg overflow-hidden w-full max-w-lg my-8">
                <div class="flex bg-[#fbfbfb] dark:bg-[#121c2c] items-center justify-between px-5 py-3">
                    ...
                    @else
                        <h5 class="font-bold text-lg text-blue-500">Add Permission</h5>
                    @endif
                </div>
                <div class="p-5 bg-gray-200 dark:bg-gray-800 text-left">
                    ...
                    >
                        <div class="mb-1">
                            <label for="Name" class="my-label">Permission Name</label>
                            <input type="text" wire:model="name" placeholder="Permission Name" id="Name" class="my-input focus:outline-none focus:shadow-outline">
                            @if ($errors->has('name'))
                                <div class="text-red-500">{{ $errors->first('name') }}</div>
                            @endif
                        </div>
                        <div class="mb-1">
                            <label for="group" class="my-label">Group Name</label>
                            <input type="text" wire:model="group" placeholder="Group Name" id="group" class="my-input focus:outline-none focus:shadow-outline">
                            @if ($errors->has('group'))
                                <div class="text-red-500">{{ $errors->first('group') }}</div>
                            @endif
                        </div>
