<div class="pt-5" x-data="modal">
    ...
    <div class="flex justify-start items-center mt-4 mb-4">
        <button @click="toggle; $wire.call('showModal')" class="bg-blue-500 font-bold btn text-white border-0 flex items-center justify-between">
    ...
    <div class="bg-white dark:bg-slate-900 shadow-md rounded px-4 md:px-8 pt-6 pb-8 mb-4 w-full">
        <h2 class="mb-2 font-bold text-3xl dark:text-white text-blue-500">Benefit Of Course</h2>
        <hr>
        <div>
            {{-- Show Data --}}
            <table class="w-full">
                ...
                <tbody>
                    @forelse ($courseBenefit as $key => $data)
                        <tr>
                            <td class="p-3 border-b border-[#ebedf2] dark:border-[#191e3a] text-center">
                                {{ $courseBenefit->firstItem() + $key }}
                            </td>
                            <td class="p-3 border-b border-[#ebedf2] dark:border-[#191e3a] text-center">
                                {{ $data->content }}
                            </td>
            ...
            <div class="livewire-pagination mt-5">{{ $courseBenefit->links() }}</div>
        </div>
    </div>
        ...
        <div class="flex items-center justify-center min-h-screen px-4" @click.self="open = false">
            <div x-show="open" x-transition x-transition.duration.400
                ...
                <div class="flex bg-[#fbfbfb] dark:bg-[#121c2c] items-center justify-between px-5 py-3">
                    @if (!empty($update_id))
                        <h5 class="font-bold text-lg text-blue-500">Update Benefit</h5>
                    @else
                        <h5 class="font-bold text-lg text-blue-500">Add Benefit</h5>
                    @endif
                </div>
                <div class="p-5 bg-gray-200 dark:bg-gray-800 text-left">
                    <form method="post"
                        ...
                        <div class="mb-1">
                            <label for="learnings" class="my-label text-blue-500">Learnings</label>
