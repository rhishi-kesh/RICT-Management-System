<div class="pt-5" x-data="modal">
    ...
    <div class="flex justify-start items-center mt-4 mb-4">
        ...
        <a href="{{ route('course') }}" class="ml-4 btn btn-back">Back</a>
    </div>

    <div class="bg-white dark:bg-slate-900 shadow-md rounded px-4 md:px-8 pt-6 pb-8 mb-4 w-full">
        <h2 class="mb-2 font-bold text-3xl dark:text-white text-blue-500">Course Learnings</h2>
        <hr>
        <div>
            {{-- Show Data --}}
            <table class="w-full">
                ...
                <tbody>
                    @forelse ($courseLearning as $key => $data)
                        <tr>
                            <td class="p-3 border-b border-[#ebedf2] dark:border-[#191e3a] text-center">
                                {{ $courseLearning->firstItem() + $key }}
                            </td>
                            <td class="p-3 border-b border-[#ebedf2] dark:border-[#191e3a] text-center">
                                {{ $data->content }}
                            </td>
                            <td class="p-3 border-b border-[#ebedf2] dark:border-[#191e3a] text-center h-14 w-14">
                                <img class="shadow-[0_0_15px_1px_rgba(113,106,202,0.30)] dark:shadow-none" src="{{ asset('storage/' . $data->image) }}"  width="500" height="200" alt="img" />
                            </td>
            ...
            <div class="livewire-pagination mt-5">{{ $courseLearning->links() }}</div>
        </div>
    </div>
        ...
        <div class="flex items-center justify-center min-h-screen px-4" @click.self="open = false">
            <div x-show="open" x-transition x-transition.duration.400
                ...
                <div class="flex bg-[#fbfbfb] dark:bg-[#121c2c] items-center justify-between px-5 py-3">
                    @if (!empty($update_id))
                        <h5 class="font-bold text-lg text-blue-500">Update Learning</h5>
                    @else
                        <h5 class="font-bold text-lg text-blue-500">Add Learning</h5>
                    @endif
                </div>
